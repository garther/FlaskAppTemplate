...
# this flag controls how permanent sessions are refreshed. If set to True
# (which is the default) then the cookie is refreshed each request which
# automatically bumps the lifetime. If set to False a set-cookie header is only
# sent if the session is modified.
...
# the policy of the default logging handler. The default is 'always' which
# means that the default logging handler is always active. 'debug' will only
# activate logging in debug mode, 'production' will only log in production and
...
# be useful to figure out why templates cannot be found or wrong templates
# appear to be loaded.
# EXPLAIN_TEMPLATE_LOADING = FalseF
